...
$csv = array_map('str_getcsv', file('bii_guests.csv'));
...
foreach($csv as $key => $row){
    ...
    if($key != 0){
        ...
        if(!empty($currFirstname) && !empty($currSurname)){
            ...
            $person = "{\"COMPANY\" : \"{$currCompany}\", \"TABLE_NUMBER\" : {$currTableNumber}, \"FIRSTNAME\" : \"{$currFirstname}\", \"SURNAME\" : \"{$currSurname}\", \"JOB_TITLE\" : \"{$currJobTitle}\"}";
